...
def line_follower(vel, angle, obs):
    ...
    if line_white != (0, 0, 0, 0) or line_yellow != (0, 0, 0, 0):
        ...
        if angle_white[0] is not None:
            if 30 < angle_white[1] < 40 or -40 < angle_white[1] < -30:
                print(f"LINEA VERTICAL - BLANCA {round(angle_white[1], 2)}")
                rot_vel = 0
                x_vel = 0.2

            elif angle_white[1] >= 70 and angle_white[0] > 30:
    ...
    # Asegurarse de que el robot se mantenga dentro del carril
    if pos_x < -0.1 or pos_x > 0.1:
        x_vel = 0
        rot_vel = -1 if pos_x > 0 else 1
